...
public class QunChat extends QunChatFrame
{
	...
	public QunChat(Main main)
	{
		...
		initLabelEvent();
		this.setTitle("聊天室(当前用户："+main.lbl用户名.getText()+")");
		addEvent();
	...
	{
		new MyLabel(lbl文字).addEvent();
		new MyLabel(lbl表情).addEvent();
		new MyLabel(jLabel聊天记录).addEvent();
		new MyLabel(lbl视频).addEvent();
		new MyLabel(lbl语音).addEvent();
		new MyLabel(lbl发送文件).addEvent();
